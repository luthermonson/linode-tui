...
func clearAccountCommand() *cli.Command {
	return &cli.Command{
		Name:  "clear-account",
		Usage: "DESTRUCTIVE: delete every resource on a Linode account",
		Description: "Iterates known resource types (instances, volumes, nodebalancers, " +
			"LKE clusters, firewalls, domains, custom images, my stackscripts, " +
			"VPCs+subnets, placement groups, DBaaS, object storage buckets) and " +
			"deletes them. Defaults to dry-run; pass --execute to actually delete. " +
			"Refuses any account name containing \"prod\" unless --i-know-what-im-doing.",
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:     "account",
				Usage:    "named account from config to wipe",
				Required: true,
			},
			&cli.BoolFlag{
				Name:  "execute",
				Usage: "actually delete (default is dry-run)",
			},
			&cli.BoolFlag{
				Name:  "i-know-what-im-doing",
				Usage: "permit account names containing 'prod'",
			},
			&cli.StringSliceFlag{
				Name:  "exclude",
				Usage: "skip resource types (instances,volumes,nodebalancers,lke,firewalls,domains,images,stackscripts,vpcs,placementgroups,databases,objectstorage)",
			},
			&cli.StringFlag{
				Name:  "grep",
				Usage: "only act on resources whose label/name contains this substring (case-insensitive)",
			},
			&cli.StringFlag{
				Name:  "config",
		...
		Action: func(ctx context.Context, c *cli.Command) error {
			return runClearAccount(ctx, c, os.Stdout)
		},
	}
}
...
func runClearAccount(ctx context.Context, c *cli.Command, out io.Writer) error {
	opts := clearOptions{
		Account: c.String("account"),
		Execute: c.Bool("execute"),
		Force:   c.Bool("i-know-what-im-doing"),
		Exclude: c.StringSlice("exclude"),
		Grep:    c.String("grep"),
	}
	if err := clearAccountGuard(opts.Account, opts.Force); err != nil {
		return err
	}
	...
	if _, ok := cfg.Accounts[opts.Account]; !ok {
		return fmt.Errorf("account %q not found in config", opts.Account)
	}
	token, err := linode.ResolveTokenForAccount(ctx, cfg, opts.Account)
	if err != nil {
		return err
	}
	return clearAccountWithClient(ctx, linode.NewClient(token), opts, out)
}

// clearAccountGuard validates the account name. Exposed for tests.
func clearAccountGuard(name string, force bool) error {
	if name == "" {
		return fmt.Errorf("--account is required")
	}
	if strings.Contains(strings.ToLower(name), "prod") && !force {
		return fmt.Errorf("refusing to clear %q (contains 'prod'); re-run with --i-know-what-im-doing", name)
	}
	return nil
}

// clearAccountWithClient is the inner loop, testable with an httptest-backed
// client.
func clearAccountWithClient(ctx context.Context, client *linode.Client, opts clearOptions, out io.Writer) error {
...
func modeStr(dry bool) string {
	if dry {
		return "DRY-RUN (pass --execute to delete)"
	}
	return "EXECUTING"
}
...
type clearStep struct {
	kind string
	run  func(context.Context, *linode.Client, *clearWriter)
}
...
func clearInstances(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func clearVolumes(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func clearNodeBalancers(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func clearLKEClusters(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func clearFirewalls(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func clearDomains(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func clearImages(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func clearStackScripts(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func clearVPCs(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func clearPlacementGroups(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func clearDatabases(ctx context.Context, c *linode.Client, w *clearWriter) {
...
func deleteDatabaseByEngine(ctx context.Context, c *linode.Client, db linodego.Database) error {
	switch {
...
func clearObjectStorage(ctx context.Context, c *linode.Client, w *clearWriter) {
