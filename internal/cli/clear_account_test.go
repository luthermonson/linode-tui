...
func TestClearAccountGuard(t *testing.T) {
	if err := clearAccountGuard("", false); err == nil {
		t.Fatal("expected error for empty name")
	}
	if err := clearAccountGuard("prod-main", false); err == nil {
		t.Fatal("expected error for prod-* without force")
	}
	if err := clearAccountGuard("PROD", false); err == nil {
		t.Fatal("expected error for case-insensitive prod")
	}
	if err := clearAccountGuard("prod-main", true); err != nil {
		t.Fatalf("force should allow prod: %v", err)
	}
	if err := clearAccountGuard("dev", false); err != nil {
...
func TestClearAccountDryRunNoMutations(t *testing.T) {
	...
	err := clearAccountWithClient(context.Background(), client, clearOptions{
